...
# Generate a random UUID for transactionId
ANOTHER_TRANSACTION_ID=$(uuidgen)
...
ANOTHER_PAYLOAD=$(jq -n --arg tx_id "$ANOTHER_TRANSACTION_ID" '{"transactionId": $tx_id, "funds": "20.13"}')
...
# Executing another transaction for good measure
curl -X PUT -H "userID: $USER_ID" -H "Content-Type: application/json" http://localhost:8080/wallets/$WALLET_ID/funds/add -d "$ANOTHER_PAYLOAD"
